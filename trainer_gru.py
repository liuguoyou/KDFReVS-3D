...
class Trainer_GRU():
    def __init__(self, cfgs, model, get_data_loader_func):
        ...
        self.max_frame_number = cfgs.get('max_num_views', 30)
        self.min_frame_number = cfgs.get('min_num_views', 15)
        ...
        self.train_loader, self.val_loader, self.test_loader = get_data_loader_func(cfgs)
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.num_epochs):
            self.train_loader, self.val_loader, self.test_loader = self.get_data_loader_func(self.cfgs)
    ...
    def run_epoch(self, loader, epoch=0, is_validation=False, is_test=False):
        ...
        for iter, input in enumerate(loader):
            m = self.model.forward(input, valid=not is_train)
            if is_train:
                self.model.backward()
            elif is_test:
            ...
            if self.use_logger and is_train:
                ...
                if total_iter % self.log_freq == 0:
                    self.model.forward(self.viz_input, valid=True)
