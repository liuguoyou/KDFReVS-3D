...
def setup_runtime(args):
    ...
    fintune_config_path = cfgs.get("fintune_config_path", None)
    if fintune_config_path is not None and os.path.isfile(fintune_config_path):
        fcfgs = load_yaml(fintune_config_path)
        cfgs.update(fcfgs)
